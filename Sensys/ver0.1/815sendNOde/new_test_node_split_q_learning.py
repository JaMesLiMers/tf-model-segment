...
def send_time(time_name, time_num, soc, buffsize):
    soc.sendall("{}".format(time_name).encode())
    recive = soc.recv(buffsize).decode()
    if recive != "recive":
        time.sleep(0.1)
    elif recive == "recive":
        soc.sendall("{}".format(time_num).encode())
...
def send(soc, sess, time_1, time_2, preprocessed_image, buffsize):
...
if __name__ == '__main__':
    ...
    sess, preprocessed_image, time_1, time_2 = preprocess()
    while True:
        send(soc, sess, time_1, time_2, preprocessed_image, buffsize)
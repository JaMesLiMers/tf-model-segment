...
def send_weight_and_time(soc, sess, preprocessed_image, cut_point, time_1, time_2):

    buffsize = 65535

    # 传输 加载参数时间
    soc.send("time_1".encode())
    recive = soc.recv(buffsize).decode()
    if recive != "recive":
        time.sleep(0.1)
    elif recive == "recive":
        soc.send("{}".format(time_1).encode())
    ...
    # 传输 预处理时间
    soc.send("time_2".encode())
    recive = soc.recv(buffsize).decode()
    if recive != "recive":
        time.sleep(0.1)
    elif recive == "recive":
        soc.send("{}".format(time_2).encode())
        done = soc.recv(buffsize).decode()
        if done != "done":
            time.sleep(0.1)
        else:
...
def main(_):
    ...
    for i in range(9):
        ...
        send_weight_and_time(soc, sess, preprocessed_image, i+1, time_1, time_2)

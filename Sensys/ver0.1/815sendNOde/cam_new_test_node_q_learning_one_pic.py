...
def send_weight_and_time(sess, preprocessed_image, soc, buffsize):
    # 开始传输
    soc.sendall("send_weight_cutpoint".encode())
    ...
    if recive == None:
        ...
        # 传输后传输 cut point
        soc.sendall("{}%".format(cut_point).encode())
...
def preprocess(sess, time_1, input_image):
    ...
    print('Preprocessing...')
    preprocessed_image, original_image = preprocessing(input_image, input_height, input_width)

    # time_2 为预处理时间
    time_2 = time.time() - time_2_1

    return sess, preprocessed_image, original_image, time_1, time_2
...
def send(soc, sess, time_1, time_2, preprocessed_image, original_image, buffsize):
    ...
    send_weight_and_time(sess, preprocessed_image, soc, buffsize)


def main():
    sess, time_1 = load_data()
    video_capture = enable_video_capture(0)
    time.sleep(2)
    _, input_image = video_capture.read()
    soc, buffsize = connect()
    while True:
        sess, preprocessed_image, original_image, time_1, time_2 = preprocess(sess, time_1, input_image)
        send(soc, sess, time_1, time_2, preprocessed_image, original_image, buffsize)
